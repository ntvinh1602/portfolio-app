...
Deno.serve(async (_req: Request) => {
  ...
  try {
    ...
    const tickers = securities.map(s => `${s.ticker}-USD`);
    ...
    securities.forEach(security => {
      const suffixedTicker = `${security.ticker}-USD`;
    ...
    if (successfulFetches.length > 0) {
      ...
      if (upsertError) {
        console.error('Error saving crypto prices:', upsertError);
...
🪙 Crypto Update (yahoo-finance2) - ${formattedDate}
