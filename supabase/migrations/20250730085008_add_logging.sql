...
BEGIN
    FOR loop_date IN SELECT generate_series(p_start_date, p_end_date, '1 day'::interval)::date LOOP
        ...
        RAISE LOG 'DEBUG: Snapshot for user %, date %', p_user_id, loop_date;
        ...
        FOR v_row IN SELECT * FROM temp_cash_flow_legs LOOP
            RAISE LOG 'DEBUG: Found leg for transaction %: type=%, asset_class=%, amount=%', v_row.transaction_id, v_row.type, v_row.asset_class, v_row.amount;
        END LOOP;
        ...
        RAISE LOG 'DEBUG: Calculated net_cash_flow for % on %: %', p_user_id, loop_date, v_net_cash_flow;
