...
create or replace view "flight"."flights_readable" with
  (security_invoker = on) as  SELECT f.user_id,
    f.flight_number,
    f.tail_number,
    f.departure_time,
    f.arrival_time,
...
create or replace view "flight"."lifetime_stats" with
  (security_invoker = on) as  WITH visited_airports AS (
