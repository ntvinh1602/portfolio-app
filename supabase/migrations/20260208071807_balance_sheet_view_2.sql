...
-- Apply GREATEST(0, cash)
assets_fixed AS (
  SELECT
    GREATEST(cash_mv_raw, 0) AS cash_mv,
    stock_mv,
    fund_mv,
    crypto_mv,
    cash_cb,
...
equity AS (
  SELECT
    ...
    (
      (SELECT (cash_mv + stock_mv + fund_mv + crypto_mv) FROM assets_fixed)
      - (SELECT (cash_cb + stock_cb + fund_cb + crypto_cb) FROM assets_fixed)
