-- Drops the existing function to ensure a clean recreation.
DROP FUNCTION IF EXISTS public.get_transaction_feed();

-- Recreates the function with the correct return signature and query structure.
...
BEGIN
    ...
    SELECT
        ...
        tl.quantity,
        tl.amount,
        tl.currency_code::text -- Explicitly cast to text for safety
    FROM
