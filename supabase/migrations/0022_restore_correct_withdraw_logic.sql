CREATE OR REPLACE FUNCTION public.handle_withdraw_transaction(
    ...
    p_amount numeric,
    p_description text,
    p_asset_id uuid -- This is the cash asset being withdrawn from
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
    ...
    IF v_draw_from_capital > 0 THEN
        ...
        IF v_draw_from_capital > v_capital_balance THEN
            RETURN jsonb_build_object(
                'error', 'Withdrawal amount exceeds available capital. See details for debug info.',
                'details', jsonb_build_object(
                    'p_amount', p_amount,
                    'v_earnings_balance', v_earnings_balance,
                    'v_draw_from_earnings', v_draw_from_earnings,
                    'v_draw_from_capital', v_draw_from_capital,
                    'v_capital_balance', v_capital_balance,
                    'failed_check', format('%s > %s', v_draw_from_capital, v_capital_balance)
                )
            );
        END IF;
    END IF;
