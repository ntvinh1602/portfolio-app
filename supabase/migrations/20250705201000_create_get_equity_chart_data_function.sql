CREATE OR REPLACE FUNCTION get_equity_chart_data(
    p_user_id UUID,
    p_start_date DATE,
    p_end_date DATE,
    p_threshold INT
...
BEGIN
    ...
    IF data_count <= p_threshold THEN
        RETURN QUERY SELECT rd.snapshot_date, rd.net_equity_value FROM raw_data rd;
        DROP TABLE raw_data;
        RETURN;
    END IF;
    ...
    INSERT INTO result_data_temp SELECT rd.snapshot_date, rd.net_equity_value FROM raw_data rd WHERE rn = 1;
    ...
    FOR i IN 0..p_threshold - 3 LOOP
        ...
        SELECT AVG(EXTRACT(EPOCH FROM rd.snapshot_date)) INTO avg_x FROM raw_data rd WHERE rn >= range_start AND rn <= range_end;
        ...
        FOR data IN SELECT * FROM raw_data WHERE rn >= range_start AND rn <= range_end LOOP
            point_area := abs(
                (EXTRACT(EPOCH FROM result_data.snapshot_date) - avg_x) * (data.net_equity_value - result_data.net_equity_value) -
                (EXTRACT(EPOCH FROM result_data.snapshot_date) - EXTRACT(EPOCH FROM data.snapshot_date)) * (avg_y - result_data.net_equity_value)
            ) * 0.5;
        ...
        INSERT INTO result_data_temp (snapshot_date, net_equity_value)
        VALUES (point_to_add.snapshot_date, point_to_add.net_equity_value);

        a := a + 1;
    END LOOP;

    -- Always add the last point
    INSERT INTO result_data_temp SELECT rd.snapshot_date, rd.net_equity_value FROM raw_data rd WHERE rn = data_count;

    RETURN QUERY SELECT * FROM result_data_temp ORDER BY snapshot_date;
