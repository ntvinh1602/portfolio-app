drop function if exists "public"."handle_sell_transaction"("uuid", "uuid", numeric, numeric, "date", "uuid", "uuid", "text");

CREATE OR REPLACE FUNCTION "public"."handle_sell_transaction"("p_user_id" "uuid", "p_asset_id" "uuid", "p_quantity_to_sell" numeric, "p_price" numeric, "p_transaction_date" "date", "p_cash_account_id" "uuid", "p_cash_asset_id" "uuid", "p_description" "text", "p_created_at" timestamptz default now()) RETURNS "uuid"
    LANGUAGE "plpgsql"
