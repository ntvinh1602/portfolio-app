...
ALTER TYPE public.equity_point DROP ATTRIBUTE cumulative_cashflow;
ALTER TYPE public.equity_point DROP ATTRIBUTE net_equity;
ALTER TYPE public.equity_point ADD ATTRIBUTE total_cashflow numeric;
ALTER TYPE public.equity_point ADD ATTRIBUTE total_equity numeric;
...
BEGIN
  ...
  RETURN v_pnl;
END;$function$;
CREATE OR REPLACE FUNCTION public.get_equity_chart(p_start_date date, p_end_date date, p_threshold integer DEFAULT 150)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE VIEW public.equity_return_data WITH (security_invoker=true) AS WITH periods AS (
         SELECT CURRENT_DATE AS today,
            ...
            ( SELECT (min(tx_entries.created_at))::date AS min
                   FROM public.tx_entries) AS inception_date,
...
CREATE MATERIALIZED VIEW public.daily_snapshots AS WITH users AS (
         ...
         SELECT b.snapshot_date,
            ...
            b.total_equity,
            b.intraday_cashflow,
            round(COALESCE(sum(b.intraday_cashflow) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), (0)::numeric)) AS total_cashflow,
                CASE
                    WHEN (lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date) IS NULL) THEN (0)::numeric
                    WHEN (lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date) = (0)::numeric) THEN (0)::numeric
                    ELSE ((b.total_equity - b.intraday_cashflow) - lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date))
                END AS intraday_pnl,
                CASE
                    WHEN (lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date) IS NULL) THEN (0)::numeric
                    WHEN (lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date) = (0)::numeric) THEN (0)::numeric
                    ELSE (((b.total_equity - b.intraday_cashflow) - lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date)) / NULLIF(lag(b.total_equity) OVER (PARTITION BY b.user_id ORDER BY b.snapshot_date), (0)::numeric))
