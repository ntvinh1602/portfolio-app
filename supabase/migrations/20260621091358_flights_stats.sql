...
CREATE OR REPLACE VIEW "flight"."lifetime_stats" WITH ("security_invoker"='on') AS
 SELECT "f"."flights_count",
    "a"."airports_count",
    "c"."country_count",
    "f"."airframe_count",
    "f"."total_distance",
    "f"."total_duration"
   FROM ((( SELECT "count"(*) AS "flights_count",
            "count"(DISTINCT "flights"."aircraft_id") AS "airframe_count",
            ( SELECT "sum"("flights_readable"."distance_km") AS "sum"
                   FROM "flight"."flights_readable") AS "total_distance",
            ("round"((EXTRACT(epoch FROM "sum"(("flights"."arrival_time" - "flights"."departure_time"))) / (3600)::numeric), 1) || ' hours'::"text") AS "total_duration"
           FROM "flight"."flights") "f"
     CROSS JOIN ( SELECT "count"(DISTINCT "airports"."airport_id") AS "airports_count"
           FROM ( SELECT "flights"."departure_airport_id" AS "airport_id"
                   FROM "flight"."flights"
                UNION ALL
                 SELECT "flights"."arrival_airport_id"
                   FROM "flight"."flights") "airports") "a")
     CROSS JOIN ( SELECT "count"(DISTINCT "a_1"."country") AS "country_count"
           FROM ("flight"."flights" "f_1"
             JOIN "flight"."airports" "a_1" ON ((("a_1"."id" = "f_1"."departure_airport_id") OR ("a_1"."id" = "f_1"."arrival_airport_id"))))) "c");
