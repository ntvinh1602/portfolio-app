-- Drop the existing function to redefine it with new parameters for filtering
DROP FUNCTION IF EXISTS public.get_transaction_feed(integer, integer);

-- Recreate the function with pagination and filtering parameters
CREATE OR REPLACE FUNCTION public.get_transaction_feed(
    page_size integer,
    page_number integer,
    start_date date DEFAULT NULL,
    end_date date DEFAULT NULL,
    asset_class_filter text DEFAULT NULL
)
RETURNS TABLE(
    transaction_id uuid,
...
BEGIN
    ...
    WHERE
        ...
        (asset_class_filter IS NULL OR a.asset_class = asset_class_filter)
    ORDER BY
