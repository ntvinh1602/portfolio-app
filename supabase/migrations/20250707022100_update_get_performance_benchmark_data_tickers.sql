...
-- Recreate the function with corrected benchmark tickers
CREATE OR REPLACE FUNCTION public.get_performance_benchmark_data(p_user_id uuid, p_start_date date, p_end_date date)
RETURNS TABLE(date text, portfolio_value numeric, gspc_value numeric, vni_value numeric)
LANGUAGE plpgsql
AS $$
DECLARE
    v_first_portfolio_value numeric;
    v_first_gspc_value numeric;
...
BEGIN
    ...
    SELECT md.close INTO v_first_gspc_value
    FROM market_data md
    WHERE md.symbol = '^GSPC' AND md.date >= p_start_date
    ORDER BY md.date
    LIMIT 1;
    ...
    gspc_data AS (
        SELECT
            ...
            md.close
        FROM market_data md
        WHERE md.symbol = '^GSPC' AND md.date BETWEEN p_start_date AND p_end_date
    ),
    vni_data AS (
        SELECT
            md.date,
    ...
    SELECT
        ...
        -- Normalize GSPC value
        (gspc.close / v_first_gspc_value) * 100 as gspc_value,
        -- Normalize VNI value
        (vni.close / v_first_vni_value) * 100 as vni_value
    FROM date_series ds
    LEFT JOIN portfolio_data pd ON ds.day = pd.date
    LEFT JOIN gspc_data gspc ON ds.day = gspc.date
    LEFT JOIN vni_data vni ON ds.day = vni.date
    WHERE pd.equity_index IS NOT NULL OR gspc.close IS NOT NULL OR vni.close IS NOT NULL -- Only return days with any data
    ORDER BY ds.day;
